...
4-1. 총 예상 비용은 해당 일정추천의 모든 날짜별 예상 비용을 합산한 값이어야 함.
4-2. 일정에 나오는 **모든 장소**는 실제 존재하는 구체적인 상호명과 주소를 반드시 포함할 것. 이 말은 그 장소가 실제로 존재해야한다는 말임.
   - 예시: "속초맛집투어" 대신 `"속초 ㅇㅇ횟집 (강원 속초시 중앙로 123)"` 처럼 작성.
   - 주소는 도로명 주소 형식으로 제공.
