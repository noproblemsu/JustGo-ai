...
3. 일정은 **동선 고려**, 자동차로 **1시간 이내 이동**, 무리하지 않게 구성할 것.
4. ⚠️ **각 일정(추천 1개)의 총 예상 비용은 반드시 예산의 ±15% 이내**여야 해.
   - 예: 예산이 {budget:,}원이면, 각 일정의 총 비용은 **{int(budget*0.85):,}원 ~ {int(budget*1.15):,}원** 범위 내여야 해.
   - 이 범위를 벗어나면 무효야. 반드시 정확히 맞춰.
5. 각 날짜의 마지막에는 **간단한 마무리 문장**을 넣을 것.
6. ⚠️ **총 예상 비용은 각 일정마다 마지막에 1번만 작성**할 것.
   - ⚠️ 각 날짜별 비용 요약은 절대 쓰지 마. (예: "8월 8일 총 예상 비용은 ..." 이런 거 쓰지 마)
   - 반드시 일정 마지막 줄에만 다음처럼 출력:
     `총 예상 비용은 약 289,000원으로, 입력 예산인 300,000원 내에서 잘 계획되었어요.`
7. 출력은 **마크다운 형식**으로 예쁘게 정리할 것.
8. 총 {count}개의 추천 일정을 구성하고, **서로 다른 스타일**로 구성할 것.
9. 각 일정은 다음 제목 형식으로 시작하고, 반드시 `---` 구분선을 둘 것:
   - 일정추천 1: 힐링중심
   - 일정추천 2: 자연중심
   - 일정추천 3: 먹기중심
10. 각 제목 다음 줄부터 본문 시작. 일정 본문 중에는 `"일정추천"`이라는 단어를 다시 쓰지 마.
11. 위 규칙을 하나라도 어기면 잘못된 응답이야. 반드시 모두 지켜.
"""
