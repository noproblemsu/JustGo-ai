...
def build_prompt(location, days, budget, companions, style, selected_places, travel_date, count=3):
    ...
    # 예산 범위 계산
    min_budget = int(budget * 0.85)
    max_budget = int(budget * 1.15)
...
4. **총 예상 비용은 반드시 {min_budget:,}원 이상 {max_budget:,}원 이하로 맞춰.**
   - 이 범위를 벗어나면 무조건 틀린 출력이야. 절대 초과하거나 부족하면 안 돼.
   - 일정이 끝난 후 반드시 아래와 같은 형식으로 총 예상 비용 문장을 출력해:
     `총 예상 비용은 약 267,000원으로, 입력 예산인 300,000원 내에서 잘 계획되었어요.`
5. 하루가 끝날 때마다 **간단한 마무리 문장**으로 정리해줘.
6. 결과는 날짜별로 **마크다운 형식**으로 예쁘게 정리해줘.
7. 사용자가 일정을 고를 수 있도록 **서로 다른 스타일의 여행 일정 {count}개**를 작성해줘.
8. 각 일정은 **아래와 같은 제목 형식**으로 시작하고, **일정 간에는 마크다운 구분선 `---`** 으로 구분해:
   - `일정추천 1: {{요약 제목}}`
   - `일정추천 2: {{요약 제목}}`
   - `일정추천 3: {{요약 제목}}`
9. 제목과 본문 사이에는 줄바꿈 없이 바로 이어줘 (예: 제목 다음 줄부터 본문 시작)
10. 각 일정은 **일정추천 N:**으로 정확히 시작하고, 각 일정 내에서 `일정추천` 같은 말이 또 나오지 않도록 주의해줘.

### ❗ 중요 요약:
- 반드시 총 예상 비용이 {min_budget:,}원 ~ {max_budget:,}원 사이여야 함
- 총 비용 문장은 반드시 출력해야 함
- 이 조건을 지키지 않으면 실패야
