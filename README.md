...
* **백엔드 로직**: `여행지, 날짜, 예산, 동반자, 스타일, 선택 관광지[], 선택 맛집[]`을 종합해 현실적인 동선/소요시간/식사타임을 고려한 일정을 GPT로 생성합니다.
* **예상 비용**: 하루 단위 비용을 합산하여 **일정 총액**을 계산(예산 준수). 응답에는 **총액만** 명시하도록 통제합니다.
* **데이터 흐름 & 규칙**
...
UI는 변경된 구간만 부분 리렌더링
* **백엔드 로직**: 현 상태(itinerary state)를 시스템 메시지로 제공하고, 사용자의 수정 의도를 파싱해 **변경 diff**를 GPT로부터 받아 **서버에서 병합**합니다. 스키마(Pydantic)로 검증 → 예산 초과/시간 충돌 검사 → 충돌 시 대안 제시.
