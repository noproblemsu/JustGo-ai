...
* 🎬 **시연 영상**: https://www.youtube.com/watch?v=Nz7iE0QvGGw
...
* **프로젝트 시작 계기와 목표**
    ...
    * JustGo는 “여행 일정 짜기, 왜 이렇게 피곤할까?”라는 문제의식에서 출발했습니다.

    * 여행 계획 과정에서 드는 피로를 최소화하고, 사용자의 취향과 조건에 꼭 맞는 일정을 쉽고 빠르게 세우도록 돕는 것이 목표입니다.

* **JustGo 개발 목표**

1. 생각을 덜어주는 자동 완성
   
   - 동선·식사·예산 배분까지 자동으로 제안해, 앱을 사용하고 나면 계획이 자연스럽게 완성됩니다.

2. 최소 입력, 즉시 결과
   
   - 목적지·기간·예산 등 핵심만 입력하면 바로 여행 일정 초안을 제공합니다.

3. 대화형 생성·수정
   
   - AI와의 대화를 통해 “이곳 빼고 저곳 추가”, “2일차 저녁은 가볍게”처럼 자연어로 일정을 생성·편집합니다.
...
<p align="center">
  <img src="docs/images/1.png" width="20%" alt="홈화면 예시">
</p>

* **사용자 행동**
  * 앱 실행 시 가장 먼저 진입하는 화면  
  * `내 일정 만들기` 버튼 → 입력 단계 이동  
  * 저장된 일정 카드 클릭 → 상세 일정 확인  

* **프론트엔드 구성 요소**
  * 카드 리스트 UI: **Swiper(MIT)**  
  * 홈 아이콘: 집 모양 버튼(언제든 홈으로 복귀)  
  * 로컬 데이터 연동: `localStorage` 기반 일정 표시

* **백엔드 로직**
  * 홈 진입 시 사용자 계정별 **저장된 일정 목록 조회**  
  * 일정이 없을 경우 `빈 상태 화면(empty state)` 안내  
...
* **데이터 흐름 & 규칙**
   ...
   *  각 후보의 지도 링크를 클릭하면 네이버 지도 페이지로 이동하여 위치·길찾기 등을 직접 확인할 수 있습니다.
* **백엔드 로직**
...
* **데이터 흐름 & 규칙**
  *  사용자가 카드 UI에서 선택한 관광지/맛집 → `localStorage`에 저장
* **백엔드 로직**
...
* **일정 추천 기본 구조**
  ...
  * 이후 챗봇 대화를 통해 일정 강도(강/중/약)나 활동 시간대(오전/오후) 등을 자유롭게 수정 가능
...
* **백엔드 로직**
  ...
  *  `POST /api/itinerary/confirm` : 선택된 일정을 식별자와 함께 저장
...
* **성능/캐시**
  ...
  * 추천 응답은 목적지·필터 기준 캐시, 컨텍스트는 세션 캐시
