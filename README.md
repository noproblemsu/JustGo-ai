...
* **프론트엔드 구성 요소**
  ...
  * 로컬 데이터 연동: `localStorage` 기반 일정 표시(localStorage는 UI 즉시 반영용 캐시일 뿐, SSOT는 서버 DB)
...
* **데이터 흐름 & 규칙**
  ...
  * `localStorage`는 브라우저 임시 저장용이며, **실제 단일 진실원천(SSOT)은 서버 DB**
...
* **데이터 흐름 & 규칙**
  *  사용자가 카드 UI에서 선택한 관광지/맛집 → **서버 DB 세션에 저장 (SSOT)**
  * `localStorage`에는 페이지 전환을 위한 임시 값만 유지, **실제 데이터는 서버 DB가 기준**
* **백엔드 로직**
...
* **사용자 행동**
  ...
  * 각 항목(파란 라벨) 클릭 → 해당 입력 페이지로 이동해 수정 → 저장 시 세팅 페이지로 복귀/DB 업데이트 → 이후 단계 모두 이 값 사용
...
* **데이터 흐름 & 규칙**
  ...
  * 일정 결과 또한 서버 DB에 저장 → 홈화면/챗봇/확정 화면에서 모두 같은 값 사용
* **응답 예시**
...
* **사용자 행동**
  *  사용자가 자연어로 요청 → GPT diff 생성 → **서버 DB 업데이트 (SSOT 반영)**
